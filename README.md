...
- 위치 허용 시 좌표를 소수점 넷째 자리로 줄여 환경 추천 백엔드에 전송하며, 브라우저나 서버에 지속적으로 저장하지 않습니다.
- 촬영·선택한 사진은 분석을 위해 로컬 백엔드로 전송됩니다.
- 백엔드는 최대 10MB·4096×4096·1,600만 픽셀의 이미지만 받아 메모리에서 디코딩·추론하며 파일이나 데이터베이스에 저장하지 않습니다.
- 이미지 업로드·모델 추론·환경 추천은 각각 동시 처리 한도를 두고, 허용되지 않은 브라우저 출처는 요청 본문을 읽기 전에 거부합니다.
- 외부 환경 API 오류의 URL·쿼리·API 키는 사용자 응답이나 애플리케이션 로그에 포함하지 않습니다.
- 성공 후 사진 참조를 즉시 해제하며, 실패 시에는 사용자가 재시도하는 동안에만 프론트 메모리에 유지합니다.
- 화면의 위험도는 화장품 추천을 위한 AI 결과이며 의료 진단이 아닙니다.

현재 보안 설정은 로컬 시연용입니다. 공개 배포 시에는 HTTPS 리버스 프록시, IP/사용자별 rate limit, 운영 도메인만 담은 `CORS_ORIGINS`를 추가로 설정해야 합니다.
